@echo off
rem Set environment variable to change default encoding
...
where python >nul 2>&1 && (
  ...
  rem OpenSSL Check
  if exist "D:/OpenSSL-Win64/bin/openssl.exe" (
    echo [INSTALL] Found OpenSSL executable
  ) else (
   echo [ERROR] OpenSSL executable not found in [D:/OpenSSL-Win64/bin/openssl.exe]
  ...
  if exist "D:/Microsoft Visual Studio" (
    ...
    echo [ERROR] Microsoft Visual C++ 14.0 not found in [D:/Microsoft Visual Studio]
  ...
  set LIB=D:/OpenSSL-Win64/lib;%LIB%
  set INCLUDE=D:/OpenSSL-Win64/include;%INCLUDE%

  mkdir "C:\apply\download"
  mkdir "C:\apply\tmp"
  mkdir "C:\apply\download\screen"
